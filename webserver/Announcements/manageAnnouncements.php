...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($expiry === '') {
        ...
        $messageType = 'error';
    } elseif (strtotime($expiry) === false || strtotime($expiry) <= time()) {
                ...
                <div class="ann-item">
                    ...
                    <div class="edit-form" id="edit-<?= $a['announcement_id'] ?>">
                        <form method="POST">
                            ...
                            <div class="form-group">
                                <label>Expiry</label>
                                <input type="datetime-local" name="edit_expiry"
       							value="<?= date('Y-m-d\TH:i', strtotime($a['announce_expiry'])) ?>"
       							min="<?= date('Y-m-d\TH:i') ?>">
                            </div>
                ...
                <div class="ann-item">
                    ...
                    <div class="reopen-form" id="reopen-<?= $a['announcement_id'] ?>">
                        <form method="POST" style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                            <input type="hidden" name="action" value="reopen">
                            <input type="hidden" name="announcement_id" value="<?= $a['announcement_id'] ?>">
                            <input type="datetime-local" name="new_expiry" min="<?= date('Y-m-d\TH:i') ?>">
