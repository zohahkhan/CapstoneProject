...
if (!$reset_data || !isset($reset_data['user_id']) || $reset_data['user_id'] === null) {
    header('Location: reset_password.php?error=invalid_token');
    exit();
}

if (strlen($password) < 8) {
    header('Location: reset_password.php?token=' . urlencode($token) . '&email=' . urlencode($email) . '&error=short_password');
    exit();
}

if ($password !== $confirm_password) {
    header('Location: reset_password.php?token=' . urlencode($token) . '&email=' . urlencode($email) . '&error=no_match');
    exit();
}

$user_id = (int)$reset_data['user_id'];
$reset_id = (int)$reset_data['reset_id'];
...
$statement->bindValue(':updated_by', $user_id, PDO::PARAM_INT);
...
$statement->bindValue(':reset_id', $reset_id, PDO::PARAM_INT);
$statement->execute();
