...
<body>
	...
	<script>
		...
		// FIX: allow the page to load with token only (old flow)
		// or with token + email (newer flow).
		if (token) {
		...
		if (error === 'short_password') {
			...
			document.getElementById('message').innerHTML = '<p style="color: red;">Invalid reset request.</p>';
			document.getElementById('resetForm').style.display = 'none';
		} else if (success === '1') {
